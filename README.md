...
<img src="results/masks/IC-CX-01812-01__20200918_090206.png" width="1000px" align="center" />
<img src="results/stitches/IC-CX-01812-01__20200918_090206.png" width="1000px" align="center" />
...
```python 
if args.task == 'cirvical_40x_cv':
    args.n_classes=4
    dataset = Generic_MIL_Dataset(csv_path = 'dataset_csv/cirvical_clean.csv',
                            data_dir= os.path.join(args.data_root_dir, 'cirvical_feat_resnet'),
                            shuffle = False, 
                            seed = args.seed, 
                            print_info = True,
                            label_dict = {'Low grade CIN/HPV ':0, ' High Grade CIN':1, 'Squamous': 2, 'Other': 3},
                            label_col = 'label',
                            ignore=[])
```
...
```python
parser.add_argument('--task', type=str, choices=['cirvical_40x_cv',  'endometrium_40x_cv'])
```

### Training Splits
For evaluating the algorithm's performance, multiple folds (e.g. 10-fold) of train/val/test splits can be used. Example 10-fold 80/10/10 splits for cirvical and endometrium, using 50% of training data can be found under the **splits** folder. These splits can be automatically generated using the create_splits_seq.py script with minimal modification just like with **main.py**. For example, cirvical splits with 75% of training data can be created by calling:
 
``` shell
python create_splits_seq.py --task cirvical_40x_cv --seed 1 --label_frac 0.75 --k 10
```
...
``` shell
CUDA_VISIBLE_DEVICES=0 python main.py --drop_out --early_stopping --lr 2e-4 --k 10 --label_frac 0.5 --exp_code cirvical_40x_cv_CLAM_50 --weighted_sample --bag_loss ce --inst_loss svm --task cirvical_40x_cv --model_type clam_sb --log_data --subtyping --data_root_dir DATA_ROOT_DIR
``` 
### GPU Training Example for Binary Positive vs. Negative Classification( e.g. Camelyon)
``` shell
...
``` shell
CUDA_VISIBLE_DEVICES=0 python eval.py --drop_out --k 10 --models_exp_code cirvical_40x_cv_CLAM_50_s1 --save_exp_code cirvical_40x_cv_CLAM_50_s1_cv --task cirvical_40x_cv --model_type clam_sb --results_dir results --data_root_dir DATA_ROOT_DIR
```

``` shell
CUDA_VISIBLE_DEVICES=0 python eval.py --drop_out --k 10 --models_exp_code cirvical_40x_cv_CLAM_50_s1 --save_exp_code cirvical_40x__cv_CLAM_50_s1_cv --task cirvical_cv --model_type clam_sb --results_dir results --data_root_dir DATA_ROOT_DIR
```
