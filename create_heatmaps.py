...
from icairdweakly.utils.eval_utils import initiate_model as initiate_model
from icairdweakly.models.model_clam import CLAM_MB, CLAM_SB
from icairdweakly.models.resnet_custom import resnet50_baseline
from types import SimpleNamespace
from collections import namedtuple
import h5py
import yaml
from icairdweakly.wsi_core.batch_process_utils import initialize_df
from icairdweakly.vis_utils.heatmap_utils import initialize_wsi, drawHeatmap, compute_from_patches
from icairdweakly.wsi_core.wsi_utils import sample_rois
...
parser.add_argument('--save_dir', type=str, default=None,
                                        help='directory to save heatmap results')
...
def get_best_level_for_downsample(downsample):
    best_level = math.log(downsample, 2) 
    return best_level

def infer_single_slide(model, features, label, reverese_label_dict, k=1):
        ...
        with torch.no_grad():
                ...
                print('Y_hat: {}, Y: {}, Y_prob: {}'.format(Y_hat, label, ["{:.4f}".format(p) for p in Y_prob.cpu().flatten()])) 
                probs, ids = torch.topk(Y_prob, k)
                probs = probs[-1].cpu().numpy()
                ids = ids[-1].cpu().numpy()
                pred_str = np.array([reverese_label_dict[idx] for idx in ids])
                
        return ids, pred_str, probs, A
...
def parse_config_dict(args, config_dict):
        if args.save_dir is not None:
                config_dict['exp_arguments']['save_dir'] = args.save_dir
...
#                raise NotImplementedError
        ...
        def_seg_params = {'seg_level': 6, 'sthresh': 10, 'mthresh': 7, 'close': 4, 'use_otsu': False, 
        ...
        reverese_label_dict = {class_labels[i]: class_encodings[i] for i in range(len(class_encodings))}
        print('class labels:{} Reverse label dict={}'.format(class_labels,reverese_label_dict))
        ...
        os.makedirs(os.path.join(exp_args.save_dir, exp_args.production_save_dir), exist_ok=True)
        os.makedirs(os.path.join(exp_args.save_dir, exp_args.raw_save_dir), exist_ok=True)
        ...
        for i in range(len(process_stack)):
                ...
                if isinstance(label, str):
                        grouping = reverese_label_dict[label]
                else:
                        grouping = label
                
                p_slide_save_dir = os.path.join(exp_args.production_save_dir, exp_args.save_dir, str(grouping))
                os.makedirs(p_slide_save_dir, exist_ok=True)

                r_slide_save_dir = os.path.join(exp_args.save_dir, exp_args.raw_save_dir, str(grouping),  slide_id)
               ...
               #         print('{}: {}'.format(key, val))
                ...
                # save top n = n_classes predictions
                ...
                if data_args.process_list is not None:
                        ...
                        process_stack.to_csv('heatmaps/results/{}.csv'.format(exp_args.save_dir), index=False)
                ...
                for sample in samples:
                        if sample['sample']:
                                ...
                                sample_save_dir =  os.path.join(exp_args.production_save_dir, exp_args.save_dir, 'sampled_patches', str(tag), sample['name'])
                                ...
                                for idx, (s_coord, s_score) in enumerate(zip(sample_results['sampled_coords'], sample_results['sampled_scores'])):
                                        ...
                                        patch_request = RegionRequest(tuple(s_coord), patch_args.patch_level, (patch_args.patch_size, patch_args.patch_size))
                                        patch = np.array(wsi_object.read_region(patch_request))
                                        cv2.imwrite(os.path.join(sample_save_dir, '{}_{}_x_{}_y_{}_a_{:.3f}.png'.format(idx, slide_id, s_coord[0], s_coord[1], s_score)), patch)
                ...
                if os.path.isfile(os.path.join(r_slide_save_dir, heatmap_save_name)):
                        ...
                        heatmap = drawHeatmap(scores, coords, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, 
              ...
              #                                         int(heatmap_args.blur),int(heatmap_args.use_ref_scores), int(heatmap_args.blank_canvas), float(heatmap_args.alpha), int(heatmap_args.vis_level), 
        ...
        with open(os.path.join(exp_args.save_dir, exp_args.raw_save_dir,'config.yaml'), 'w') as outfile:
