...
def initialize_df(slides, seg_params, filter_params, vis_params, patch_params, 
        ...
        if isinstance(slides, pd.DataFrame):
                ...
                slide_ids = slides
        default_df_dict = {'slide_id': slide_ids, 'scanned_level': np.full((total),1, dtype=np.uint8), 'process': np.full((total), 1, dtype=np.uint8)}
        ...
        default_df_dict.update({
                'scanned_level': np.full((total),1, dtype=np.int8), 
