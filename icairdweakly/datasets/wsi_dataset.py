...
class Wsi_Region(Dataset):
    ...
    def __init__(self, wsi_object, top_left=None, bot_right=None, level=0, 
        ...
        if level ==0:
            self.ref_downsample = 1
        else:
            downsample = tuple(ele1 // ele2 for ele1, ele2 in zip(wsi_object.level_dimensions[0], wsi_object.level_dimensions[level]))
            self.ref_downsample = downsample[1] + downsample[3] //2
    ...
    def __getitem__(self, idx):
        ...
        patch_request = RegionRequest(tuple(coord), self.level, self.patch_size)
        patch = np.array(self.wsi.read_region(patch_request))
