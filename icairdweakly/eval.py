...
if __name__ == "__main__":
    #all_patient_results=pd.DataFrame(columns = ['slide_id', 'label', 'predictd_label'])
    all_patient_results = []
    all_results = []
    all_auc = []
    all_acc = []
    ...
    patient_df = pd.DataFrame({'patient_results':all_patient_results})
    if len(folds) != args.k:
        save_name = 'summary_partial_{}_{}.csv'.format(folds[0], folds[-1])
    else:
        save_name = 'summary.csv'
    final_df.to_csv(os.path.join(args.save_dir, save_name))
    patient_df.to_csv(os.path.join(args.save_dir, 'prediction_results.csv'))
