...
parser.add_argument('--config_file', type=str, default="config_saliency.yaml")
...
if __name__ == '__main__':
    ...
    for i in range(len(process_stack)):
        ...
        if patch_args.patch_level == 0:
            ...
            downsample = tuple(ele1 // ele2 for ele1, ele2 in
                               zip(wsi_object.level_dimensions[0], wsi_object.level_dimensions[patch_args.patch_level]))
        ...
        if not os.path.isfile(h5_path):
            _, _, wsi_object = compute_from_patches(wsi_object=wsi_object, model=model,
                                                    feature_extractor=feature_extractor, batch_size=exp_args.batch_size,
                                                    **blocky_wsi_kwargs, attn_save_path=None, feat_save_path=h5_path,
                                                    ref_scores=None)
        ...
        Y_hats, Y_hats_str, Y_probs, A = infer_single_slide(model, features, label, reverese_label_dict,
                                                            exp_args.n_classes)
    ...
    #          if heatmap_args.use_roi:
    #                  save_path_full = os.path.join(r_slide_save_dir, '{}_{}_roi_False.h5'.format(slide_id,
    # patch_args.overlap))
    ...
    # float(patch_args.overlap), int(heatmap_args.use_roi),
    #                                         int(heatmap_args.blur),int(heatmap_args.use_ref_scores),
    # int(heatmap_args.blank_canvas), float(heatmap_args.alpha), int(heatmap_args.vis_level),
    ...
    #                                                    thresh=heatmap_args.binary_thresh,  patch_size =
    # vis_patch_size,
