...
def train(datasets, cur, args):
    ...
    #Using topk as bag-level loss function
    ...
    if args.early_stopping:
        early_stopping = EarlyStopping(patience = 25, stop_epoch=50, verbose = True)
    ...
    test_results_dict, test_error, test_auc, acc_logger = summary(model, test_loader, args.n_classes)
    ...
    writer.close()
    return val_results_dict, test_results_dict, 1-val_error, val_auc, 1-test_error, test_auc 
...
def summary(model, loader, n_classes):
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        #patient_results.update({'slide_id': np.array(slide_id), 'prob': (np.argmax(probs,axis = 1)).item, 'label': label.item()})
        patient_results.append([slide_id, label.item(), (np.argmax(probs,axis=1)).item()])
        error = calculate_error(Y_hat, label)
        test_error += error
